...
async def assets_page(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    from cms.routers.assets import _thumbnail_urls_for, _tags_for, _usage_for
    ...
    usage_by_asset = await _usage_for([a.id for a in assets], db)
    usage_map = {str(aid): u.model_dump(mode="json")
                 for aid, u in usage_by_asset.items()}
